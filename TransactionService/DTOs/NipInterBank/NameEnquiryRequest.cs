...
public sealed class NameEnquiryValidator : AbstractValidator<NameEnquiryRequest>
{
    public NameEnquiryValidator()
    {
        RuleFor(x => x.SenderAccountNumber)
            .NotEmpty()
            .WithMessage("SenderAccountNumber is.")
            .Length(10)
            .WithMessage("SenderAccountNumber must be 10 characters long.")
            .Must(IsAllDigit)
            .WithMessage("SenderAccountNumber must contain only digits.");
        RuleFor(x => x.SenderBankName)
            .NotEmpty()
            .WithMessage("SenderBankName is.")
            .MinimumLength(3)
            .WithMessage("SenderBankName must be at least 3 characters long.");
        RuleFor(x => x.SenderBankNubanCode)
            .NotEmpty()
            .WithMessage("SenderBankNubanCode is.")
            .Must(IsAllDigit)
            .WithMessage("SenderBankNubanCode must contain only digits.");
        RuleFor(x => x.DestinationAccountNumber)
            .NotEmpty()
            .WithMessage("DestinationAccountNumber is.")
            .Length(10)
