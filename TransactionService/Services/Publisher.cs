...
public class EventPublisher
    (IServiceScopeFactory serviceScopeFactory, ILogger<TransactionEventPublisher> logger, IProducer<string, string> kafkaProducer
    ) : BackgroundService
{
    ...
    private async Task WaitForKafkaAsync(CancellationToken token)
    {
        var backoff = 2;
