...
public static class MessageServiceExtention
{
    public static IServiceCollection AddSMSMessageServices(this IServiceCollection services)
    {
        ...
        services
            ...
            {
                options.AccountSid =
                    Environment.GetEnvironmentVariable("TWILLO_ACCOUNT_SID")
                    ?? throw new CustomTwilloException("TWILLO_ACCOUNT_SID is not set.");

                options.AuthToken =
                    Environment.GetEnvironmentVariable("TWILLO_AUTH_TOKEN")
                    ?? throw new CustomTwilloException("TWILLO_AUTH_TOKEN is not set.");

                options.FromPhoneNumber =
                    Environment.GetEnvironmentVariable("TWILLO_FROM_PHONE_NUMBER")
        ...
        // services.AddOptions<TwilioSettings>().ValidateDataAnnotations().ValidateOnStart();
